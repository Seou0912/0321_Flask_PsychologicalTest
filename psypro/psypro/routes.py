...
def show_results():
    ...
    if not df_participants.empty:
        df_participants = (
            df_participants.groupby("created_at")
            .sum()
    ...
    df_answers = pd.DataFrame(
        [{"question_id": a.question_id, "k_answer": a.k_answer} for a in answers]
    )
    graphsJSON_answers = {}
    for question_id in df_answers["question_id"].unique():
        df_question = df_answers[df_answers["question_id"] == question_id]
        fig_answer = px.pie(
            df_question, names="k_answer", title=f"문제 {question_id} 응답 비율"
        )
